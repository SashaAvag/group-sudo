...
void List::insert(int value, unsigned int in)
{
    if ( _size == 0 || _size < in) {
        std::cout << "Error: out of range:index " << in << std::endl;
    } else if (in > _size/2) {
        Node* tmp = NULL;
        tmp = (Node*) findNodeFromEnd(_size-in);
        tmp->_value = value;
    } else {
        Node* tmp = NULL;
        tmp = (Node*) findNodeFromStart(in);
        tmp->_value = value;
    }
}
...
int List::remove(unsigned int in)
{
    if ( _size == 0 || _size < in) {
        std::cout << "Error: out of range:index " << in << std::endl;
    } else if (in > _size/2) {
